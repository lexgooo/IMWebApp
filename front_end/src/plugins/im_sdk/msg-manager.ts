...
public doPolling(proto_longPolling:any, opts:any, LONG_POLLINNG_EVENT_TYPE:any, log:any, ACTION_STATUS:any, cbErr:any) {
    proto_longPolling(
        opts,
        (resp: any) => {
            for (let i in resp.EventArray) {
                ...
                switch (e.Event) {
                    case LONG_POLLINNG_EVENT_TYPE.C2C: //c2c消息通知
                        ...
                        log.warn(
                            'longpolling: received new c2c msg'
                        )
                        //获取新消息
                        this.syncMsgs()
                        break
                    case LONG_POLLINNG_EVENT_TYPE.GROUP_COMMON: //普通群消息通知
                        log.warn(
                            'longpolling: received new group msgs'
                        )
                    ...
                    case LONG_POLLINNG_EVENT_TYPE.GROUP_TIP: //（全员广播）群提示消息
                        log.warn(
                            'longpolling: received new group tips'
                        )
                        this.handlerOrdinaryAndTipGroupMsgs(
                            e.Event,
                            e.GroupTips
                    ...
                    case LONG_POLLINNG_EVENT_TYPE.C2C_EVENT: //c2c已读消息通知
                        ...
                        log.warn(
                            'longpolling: received new c2c_event msg'
                        )
                        this.handlerC2cNotifyMsgArray(
                            e.C2cNotifyMsgArray
                        )
                        break
                    default:
    ...
    constructor(
        ipList:any, authkey:any, expireTime:any, 
        longPollingDefaultTimeOut:any, LongPollingId:any, 
        proto_getLongPollingId:any, proto_longPolling:any, 
        LONG_POLLINNG_EVENT_TYPE:any, log:any, ACTION_STATUS:any, 
        proto_bigGroupLongPolling:any, tool:any, 
        curBigGroupLongPollingRetErrorCount:any, CONNECTION_STATUS:any, 
        ConnManager:any, longPollingPackageTooLargeErrorCode:any, 
        longPollingTimeOutErrorCode:any, longPollingKickedErrorCode:any,
        proto_getMsgs:any, MsgStore:any, ctx:any, SESSION_TYPE:any,
        Session:any, Msg:any, C2C_MSG_SUB_TYPE:any, MSG_ELEMENT_TYPE:any,
        FRIEND_WRITE_MSG_ACTION:any, proto_getC2CHistoryMsgs:any,
        proto_getGroupMsgs:any, onLongPullingNotify:any, 
        onAppliedDownloadUrl:any, proto_sendMsg:any,
        FRIEND_NOTICE_TYPE:any, PROFILE_NOTICE_TYPE:any,
        GROUP_SYSTEM_TYPE:any, C2C_EVENT_SUB_TYPE:any,
        proto_logout:any, proto_getIpAndAuthkey:any,
        proto_getJoinedGroupListHigh:any, curLongPollingRetErrorCount:any,
        longPollingOffCallbackFlag:any, curLongPollingStatus:any,
        LONG_POLLING_MAX_RET_ERROR_COUNT:any, longPollingIntervalTime:any,
        GROUP_MSG_SUB_TYPE:any, GROUP_MSG_PRIORITY_TYPE:any,
        getFileDownUrl:any, GROUP_TIP_TYPE:any,
        ...
        this.setBigGroupLongPollingMsgMap = (groupId:any, count:any) => {
            let bigGroupLongPollingMsgCount = this.bigGroupLongPollingMsgMap[groupId]
            if (bigGroupLongPollingMsgCount) {
                bigGroupLongPollingMsgCount =
                    parseInt(bigGroupLongPollingMsgCount) + count
                this.bigGroupLongPollingMsgMap[groupId] = bigGroupLongPollingMsgCount
            } else {
        ...
        this.longPolling = (cbOk: any, cbErr: any) => {
            ...
            if (LongPollingId) {
                ...
                this.doPolling(proto_longPolling, opts, LONG_POLLINNG_EVENT_TYPE, log, ACTION_STATUS, cbErr)
            } else {
                        ...
                        for (let i = resp.RspMsgList.length - 1; i >= 0; i--) {
                            ...
                            switch (event) {
                                case LONG_POLLINNG_EVENT_TYPE.GROUP_COMMON: //群普通消息
                                    ...
                                    msg = this.handlerGroupMsg(msgInfo, false, false)
                        ...
                        if (msgCount > 0) {
                            ...
                            log.warn(
                                'current bigGroupLongPollingMsgMap: ' +
                                    JSON.stringify(this.bigGroupLongPollingMsgMap)
                            )
                        }
                    }
        ...
        this.init = (listeners:any, cbOk:any, cbErr:any) => {
            ...
            if (listeners.onGroupSystemNotifys) {
                this.onGroupSystemNotifyCallbacks = listeners.onGroupSystemNotifys
            } else {
                log.warn('listeners.onGroupSystemNotifys is empty')
            }
            if (listeners.onGroupInfoChangeNotify) {
                this.onGroupInfoChangeCallback = listeners.onGroupInfoChangeNotify
            } else {
                log.warn('listeners.onGroupInfoChangeNotify is empty')
            }
            if (listeners.onFriendSystemNotifys) {
                this.onFriendSystemNotifyCallbacks = listeners.onFriendSystemNotifys
            } else {
    ...
    this.handlerFriendSystemNotices = (friendSystemNotices:any, isNeedValidRepeatMsg:any) => {
    ...
    //处理新的资料系统通知
    //isNeedValidRepeatMsg 是否需要判重
    this.handlerProfileSystemNotices = (profileSystemNotices:any, isNeedValidRepeatMsg:any) => {
    ...
    */
        ...
        switch (subType) {
            case C2C_EVENT_SUB_TYPE.READED:
                ...
                if (
                    ...
                    for (
                        let i = 0,
                            l = notify.ReadC2cMsgNotify.UinPairReadArray.length;
                        i < l;
                        i++
                    ) {
    ...
    //补拉群消息
    this.getLostGroupMsgs = (groupId:any, reqMsgSeq:any, reqMsgNumber:any) => {
    ...
    //处理收到的群普通和提示消息
    this.handlerOrdinaryAndTipC2cMsgs = (eventType:any, groupMsgArray:any) => {
    ...
    //处理收到的群普通和提示消息
    this.handlerOrdinaryAndTipGroupMsgs = (eventType:any, groupMsgArray:any) => {
    ...
    //处理新的群系统消息
    //isNeedValidRepeatMsg 是否需要判重
    this.handlerGroupSystemMsgs = (groupSystemMsgs:any, isNeedValidRepeatMsg:any) => {
            ...
            if (isNeedValidRepeatMsg) {
                ...
                if (this.onGroupSystemNotifyCallbacks[reportType]) {
                    if (reportType === GROUP_SYSTEM_TYPE.READED) {
                        ...
                        for (let i = 0, l = arr.length; i < l; i++) {
                            let item = arr[i]
                            this.onGroupSystemNotifyCallbacks[reportType](item)
                        }
                    } else {
                        this.onGroupSystemNotifyCallbacks[reportType](notify)
                    }
    ...
    //处理收到的普通C2C消息
    this.handlerOrdinaryAndTipC2cMsgs = (eventType:any, C2cMsgArray:any) => {
        //处理c2c消息
        let notifyInfo = []
        let msgInfos = []
    ...
    this.updatecLongPollingStatus = (errObj:any) => {
        if (
            ...
            //累计超过一定次数
            if (
                curLongPollingRetErrorCount <= LONG_POLLING_MAX_RET_ERROR_COUNT
            ) {
                ...
                setTimeout(this.startNextLongPolling, longPollingIntervalTime) //长轮询接口报错次数达到一定值，每间隔5s发起新的长轮询
            }
        }
    }
    ...
    this.handlerGroupMsg = (msgInfo:any, isSyncGroupMsgs:any, isAddMsgFlag:any, isFinished:any = undefined) => {
        if (
            msgInfo.IsPlaceMsg ||
            !msgInfo.From_Account ||
            !msgInfo.MsgBody ||
