export default  class Long {
   ...
   private TWO_PWR_16_DBL = 1 << 16;
   private TWO_PWR_24_DBL = 1 << 24;
   private TWO_PWR_32_DBL = this.TWO_PWR_16_DBL * this.TWO_PWR_16_DBL;
   private TWO_PWR_64_DBL = this.TWO_PWR_32_DBL * this.TWO_PWR_32_DBL;
   private TWO_PWR_63_DBL = this.TWO_PWR_64_DBL / 2;
   private TWO_PWR_24 = this.fromInt(this.TWO_PWR_24_DBL);
   private ZERO = this.fromInt(0)
   private UZERO = this.fromInt(0, true)
   private ONE = this.fromInt(1)
   private UONE = this.fromInt(1, true)
   private NEG_ONE = this.fromInt(-1)
   private MAX_VALUE = this.fromBits(0xFFFFFFFF | 0, 0x7FFFFFFF | 0, false)
   private MAX_UNSIGNED_VALUE = this.fromBits(0xFFFFFFFF | 0, 0xFFFFFFFF | 0, true)
   private MIN_VALUE = this.fromBits(0, 0x80000000 | 0, false)
    ...
    public isLong(other = undefined) {
       return (this && this.__isLong__) === true;
   }

   public fromInt(value, unsigned = undefined) {
   ...
   public fromNumber(value, unsigned=undefined) {
   ...
   public fromBits(lowBits, highBits, unsigned) {
       return new Long(lowBits, highBits, unsigned);
   }

   public fromString(str, unsigned = undefined, radix = undefined) {
       if (str.length === 0)
   ...
   // 下面方法转换之前是挂在 Long.prototype 下面的
   public toInt() {
   ...
   public toString(radix = 10) {
       ...
       var radixToPower = this.fromNumber(this.pow_dbl(radix, 6), this.unsigned),
           rem = this;
       var result = '';
       while (true) {
   ...
   public greaterThanOrEqual(other) {
       return this.comp( /* validates */ other) >= 0;
   };
   ...
   public negate() {
       if (!this.unsigned && this.eq(this.MIN_VALUE))
           return this.MIN_VALUE;
   ...
   public add(addend) {
       ...
       c48 += a48 + b48;
       c48 &= 0xFFFF;
       return this.fromBits((c16 << 16) | c00, (c48 << 16) | c32, this.unsigned);
   };

   public subtract(subtrahend) {
       if (!this.isLong(subtrahend))
           subtrahend = this.fromValue(subtrahend);
   ...
   public multiply(multiplier) {
       ...
       if (!this.isLong(multiplier))
           multiplier = this.fromValue(multiplier);
   ...
   public divide(divisor) {
       if (!this.isLong(divisor))
           divisor = this.fromValue(divisor);
       if (divisor.isZero())
       ...
       if (!this.unsigned) {
           ...
           if (this.eq(this.MIN_VALUE)) {
               if (divisor.eq(this.ONE) || divisor.eq(this.NEG_ONE))
                   return this.MIN_VALUE; // recall that -MIN_VALUE == MIN_VALUE
   ...
   public and(other) {
       if (!this.isLong(other))
           other = this.fromValue(other);
       return this.fromBits(this.low & other.low, this.high & other.high, this.unsigned);
   };
   ...
   public shiftLeft(numBits) {
       if (this.isLong(numBits))
           numBits = numBits.toInt();
       if ((numBits &= 63) === 0)
           return this;
       else if (numBits < 32)
           return this.fromBits(this.low << numBits, (this.high << numBits) | (this.low >>> (32 - numBits)), this.unsigned);
       else
           return this.fromBits(0, this.low << (numBits - 32), this.unsigned);
   };
   public shl = this.shiftLeft

   public shiftRight(numBits) {
       if (this.isLong(numBits))
           numBits = numBits.toInt();
       if ((numBits &= 63) === 0)
           return this;
       else if (numBits < 32)
           return this.fromBits((this.low >>> numBits) | (this.high << (32 - numBits)), this.high >> numBits, this.unsigned);
       else
           return this.fromBits(this.high >> (numBits - 32), this.high >= 0 ? 0 : -1, this.unsigned);
   };
   ...
   public shiftRightUnsigned(numBits) {
       ...
       if (numBits === 0)
           ...
           if (numBits < 32) {
               var low = this.low;
               return this.fromBits((low >>> numBits) | (high << (32 - numBits)), high >>> numBits, this.unsigned);
           } else if (numBits === 32)
               return this.fromBits(high, 0, this.unsigned);
           else
               return this.fromBits(high >>> (numBits - 32), 0, this.unsigned);
       }
   };
